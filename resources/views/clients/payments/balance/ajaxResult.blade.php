        ...
        <table class="table">
            <tbody>
                ...
                <?php $i = 0; foreach($months as $singlemonth){ 
                    ...
                    ?>
                    @php 
                    $amount = App\Models\user\Payment::where('year',$year)->where('month',$month)->where('approval_status','Approved')->where('client_id', $clientId)->sum('amount');
                    @endphp
                    <td> 
                        @if($amount > 0)
                ...
                $(document).ready(function() {
                    ...
                    $("#ShowPaymentsDetails<?php echo $i; ?>").click(function(){
                        ...
                        $.ajax({
                            method: "GET",
                            url: "{{ url('client/yearwise/monthwise/payments') }}",
                            data: "year="+$("#year<?php echo $i; ?>").val()+"&month="+$("#month<?php echo $i; ?>").val(),
...
@if($optype == "yearMonthWise")
    @php
        ...
        $monthsSheet = App\Models\user\Payment::with('client')->where('year',$year)->where('month',$month)->where('approval_status','Approved')->where('client_id', $clientId)->get();
        //dd($monthsSheet);
    @endphp
